...
topl_SAMPL_all_mat=topl_features_df

SAMPL_df=pd.read_csv(os.path.join(save_dir, dataset_name + '_SMILES.csv')) # has smiles strings

with open(os.path.join(save_dir,x_data_file_name), 'r') as read_obj:
    # pass the file object to reader() to get the reader object
    csv_reader = reader(read_obj)
    # Pass reader object to list() to get a list of lists
    topl_SAMPL_all_list = list(csv_reader)
    print(topl_SAMPL_all_list)

topl_SAMPL_all_list = np.array(topl_SAMPL_all_list)

with open(os.path.join(save_dir,y_data_file_name), 'r') as read_obj:
    # pass the file object to reader() to get the reader object
    csv_reader = reader(read_obj)
    # Pass reader object to list() to get a list of lists
    targets_topl_SAMPL_all_list = list(csv_reader)
    #print(targets_topl_SAMPL_all_list)
...
MD_list = read_extra_data(save_dir,dataset_name, Failures,
                    filename_rider="_MD.csv")
...
pca = PCA(n_components=num_of_topol_features)
principalComponents_large = pca.fit_transform(topl_SAMPL_all_list)
####################################################################################################################
#SMILES_list = SAMPL_df['smiles']
...
if make_hdf5:
    ...
    if True:
        ...
        if num_of_molecules_override == 0:
            # do all proteins woo
            num_of_molecules_to_do=len(topl_SAMPL_all_list)
        ...
        MD_ds = h.create_or_recreate_dataset(outfile, "MD", (num_of_molecules_to_do), dtype=np.float32)
        ...
        for mol_idx in range(num_of_molecules_to_do):
            ...
            this_num = MD_list[mol_idx]
            MD_ds[mol_idx] = np.array(this_num, dtype='f8')
            ...
            P_pers_img_1_ds[mol_idx],P_pers_img_2_ds[mol_idx],P_pers_img_3_ds[mol_idx]
             ) = np.array(topl_SAMPL_all_list[mol_idx], dtype='f8')
                #                           PCs                         #
            ...
            try:
                ...
                target_ds[mol_idx] = np.array([int(float(x)) for x in targets_topl_SAMPL_all_list[mol_idx]])
            except TypeError as e:
                raise (e)
            #target_norm_ds[mol_idx] = np.array(targets_topl_SAMPL_all_list[mol_idx], dtype='f8')
