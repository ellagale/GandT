...
data_dir=r'F:\Nextcloud\science\Datasets\pdbbind\v2020\PDBBind_v2020_general' ### all in the same directory
...
refined_data_file_name="INDEX_general_PL_data.2020"
...
#####################################   Settings: Alter this    ######################################################
######################################################################################################################
make_dataset=True #True # whether to recalc the dataset the dataset - saves as csv
...
refined_remaining = 0 #0#2 #0 #2 # used to make refined csv file
...
df_index_refined, df_data_refined, df_cluster_refined = h.read_in_PDBBind_data(
    ...
    data_column_list_name = ['PDB_code', 'resolution', 'release_year', '-logKd/Ki', 'Kd/Ki', 'meh', 'reference', 'ligand_name'])#["line_no.", "PDB_code", "release_year", "EC_number", "protein_name"],
...
####################################################################################################
################################     does topological features for core#############################
####################################################################################################
...
####################################################################################################
############################    Does refined topological features   ################################
####################################################################################################
...
if refined_remaining == 0:
   remaining = len(df_data_refined)
else:
    remaining = refined_remaining

csv_output_file='general_topological_features.csv'
if make_dataset:
...
if make_hdf5 & make_core_dataset:
    #?pdb
    ...
    if True:
        ...
        if num_of_core_proteins_override == 0:
            # do all proteins woo
            Num_of_proteins= len(PDB_List)
        else:
            Num_of_proteins = num_of_core_proteins_override
      ...
      #  ligand_ds = outfile.create_dataset('ligand_name', (Num_of_proteins,), dtype=string_type)
        ...
        for mol_idx in range(Num_of_proteins):
          ...
          #  protein_name_ds[mol_idx] = protein_name
            ...
            kd_ki = current_data_row.iloc[0]['original_Kd/Ki']
...
####################################################################################################
#                               Does refined hdf5 features                                          #
####################################################################################################


topl_PDB_all_refined_mat_large = np.genfromtxt(os.path.join(save_dir,csv_output_file), delimiter=',')
...
if make_hdf5:
    ...
    if True:
            ...
            release_year_ds = h.create_or_recreate_dataset(outfile, 'release_year', (Num_of_proteins,), dtype=string_type)
            #ec_number_ds = h.create_or_recreate_dataset(outfile, 'EC_number', (Num_of_proteins,), dtype=string_type)
            #ligand_name_ds = h.create_or_recreate_dataset(outfile, 'ligand_name', (Num_of_proteins,), dtype=string_type)
            logkd_ki_ds = h.create_or_recreate_dataset(outfile, '-logKd_over_Ki', (Num_of_proteins,), dtype=np.float32)
            kd_ki_ds = h.create_or_recreate_dataset(outfile, 'Kd_over_Ki', (Num_of_proteins,), dtype=string_type)
            reference_ds = h.create_or_recreate_dataset(outfile, 'reference', (Num_of_proteins,), dtype=string_type)
            if do_cluster:
    ...
    outfile.close()
    df = pd.DataFrame(actual_pdb_list)
    df.to_csv(os.path.join(save_dir,'actual_general_pdb_list.csv'))
