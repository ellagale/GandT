...
results_dir=r"C:\Users\eg16993\OneDrive - University of Bristol\Documents\Results\graphs_and_topology"
test_file='lipo.csv'
out_file_name='lipo_topological_features.hdf5'
...
#make_dataset = True
if make_dataset:
    with open(os.path.join(save_dir,'x_data_lipo.csv'), 'w') as f:
        with open(os.path.join(save_dir, 'y_data_lipo.csv'), 'w') as y_fh:
            # train
...
#                               load data                                                                        #
##################################################################################################################
topl_features_df = pd.read_csv(os.path.join(save_dir,'x_data_lipo.csv'))
topl_targets_df = pd.read_csv(os.path.join(save_dir,'y_data_lipo.csv'))

topl_lipo_all_mat=topl_features_df

lipo_df=pd.read_csv(os.path.join(save_dir,'lipo_SMILES.csv')) # has smiles strings

with open(os.path.join(save_dir,'x_data_lipo.csv'), 'r') as read_obj:
...
with open(os.path.join(save_dir,'y_data_lipo.csv'), 'r') as read_obj:
    # pass the file object to reader() to get the reader object
    csv_reader = reader(read_obj)
    # Pass reader object to list() to get a list of lists
...
with open(os.path.join(save_dir,'lipo_SMILES.csv'), 'r') as read_obj:
    # pass the file object to reader() to get the reader object
    csv_reader = reader(read_obj)
    # Pass reader object to list() to get a list of lists
...
if make_hdf5:
    ...
    if True:
        ...
        for mol_idx in range(num_of_molecules_to_do):
            ...
            target_ds[mol_idx] = np.array(targets_topl_lipo_all_list[mol_idx], dtype='f8')
